...
const BalanceCalculator = (Money) =>  { 
        if (!Array.isArray(Money) || Money.length === 0 || Array.isArray(Money) === undefined ) return  undefined
       else if (Array.isArray(Money) ) { 
            let Balance = Money[0] , withdraw = Money[1] 
            if ( withdraw <= Balance)  {return Balance -=withdraw 
